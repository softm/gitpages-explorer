(function (window, document) {
    ...
    function syncMobilePreviewSheet(section, item) {
        ...
        const shouldOpen = section.dataset.previewVisible === 'true'
            && isMobilePreviewSheetViewport()
            && item
            && item.kind === 'file';
    ...
    function createDirectoryBrowserItem(item, highlightKeywords, section) {
        ...
        element.addEventListener('pointerdown', (event) => {
            ...
            if (event.pointerType === 'touch') {
                ...
                mobileLongPressTimer = setTimeout(() => {
                    closeDirectoryActionMenu();
                    setDirectorySelection(section, [item], { previewItem: item, focus: true, skipHistory: true });
                    syncMobilePreviewSheet(section, item); // SOFTM-mobile-longpress-sheet 2026-05-15: 모바일 길게 누르기는 컨텍스트 메뉴 대신 하단 뷰어 시트를 표시
                }, 520);
                return;
            }
        ...
        if (item.kind === 'file') {
            ...
            openButton.addEventListener('click', (event) => {
                ...
                event.stopPropagation();
                openDirectoryViewer(section, item);
            }); // SOFTM-viewer-button-default 2026-05-15: 파일 링크 내부 뷰어 버튼 클릭 시 다운로드/새창 기본 동작이 함께 실행되지 않게 차단
