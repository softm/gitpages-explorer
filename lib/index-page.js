(function (window, document) {
    ...
    function getVisibleDefaultItems() {
        return defaultItems.filter(({ note }) => isNotePublishingVisible(note));
    }
    ...
    function refreshPublishingViews() {
        ...
        if (Number.isInteger(currentSelectedIndex) && notes[currentSelectedIndex]) {
            if (isNotePublishingVisible(notes[currentSelectedIndex])) {
    ...
    function buildDirectoryBrowserItems(note) {
        ...
        })).concat(files.filter(file => isPublishingVisible(file.path || file.name || '', file.security)).map(file => ({
            kind: 'file',
    ...
    function openDirectoryActionMenu(section, item, anchor, options = {}) {
        ...
        if (publishingManager && publishingManager.isAdmin() && menuItems.length > 0) {
            ...
            menu.appendChild(createDirectoryMenuButton(hasPrivate ? '퍼블리싱 ON' : '퍼블리싱 OFF', () => {
                menuItems.forEach(candidate => {
                    const path = candidate.path || candidate.name || '';
                    const status = publishingManager.getStatus(path, candidate.security);
                    const targetPath = hasPrivate && status.matchedGrantPath ? status.matchedGrantPath : path; // SOFTM-publishing-github 2026-07-09: 상위 미게시 항목은 실제 grant 경로를 메뉴 동작 대상으로 사용
                    publishingManager.setPublished(targetPath, hasPrivate);
                });
    ...
    function createDirectoryBrowserItem(item, highlightKeywords, section) {
        ...
        if (publishingToggle) {
            ...
            element.appendChild(publishingToggle);
        }
    ...
    function searchFiles() {
        ...
        if (!rawFilter && !hasActiveAttachmentFilters) {
            currentHighlightKeywords = [];
            const visibleItems = getVisibleDefaultItems();
        ...
        const filteredItems = notes.filter(isNotePublishingVisible).filter(note => {
