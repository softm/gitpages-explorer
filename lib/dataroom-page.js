;(function (window, document) {
    ...
    function updateMobileActionBar(item, index, selectedCount = 1) {
        ...
        if (selectedCount > 1) {
            ...
            const selectedFiles = selectedItems.filter(candidate => candidate && candidate.kind === 'file');
            meta.textContent = [
                selectedFiles.length ? `파일 ${selectedFiles.length.toLocaleString()}개` : '',
                selectedItems.length - selectedFiles.length ? `디렉토리 ${(selectedItems.length - selectedFiles.length).toLocaleString()}개` : ''
            ...
            return;
        } // SOFTM-dataroom-multi-actions 2026-05-17: 모바일 하단 액션 바에도 다중 다운로드/선택 해제 제공
