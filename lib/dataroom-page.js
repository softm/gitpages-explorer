;(function (window, document) {
    ...
    function openDataRoomActionMenu(item, anchor, options = {}) {
        ...
        menu.setAttribute('aria-label', '자료실 항목 작업');

        const title = document.createElement('div');
        title.className = 'directory-action-menu-title';
        title.textContent = item.kind === 'directory' ? '디렉토리 작업' : '파일 작업';
        ...
        if (item.kind === 'file') {
            ...
            menu.appendChild(createDataRoomMenuButton('경로 복사', () => copyDataRoomText(getDataRoomItemPath(item))));
            menu.appendChild(createDataRoomMenuButton('URL 복사', () => copyDataRoomText(getDataRoomItemAbsoluteUrl(item))));
        } else {
            menu.appendChild(createDataRoomMenuButton('메인', () => {
                window.location.href = getDataRoomNoteUrl(item);
            }));
    ...
    function startDataRoomDragSelection(item, index, event, node) {
        if (!item || !event || event.button !== 0 || event.pointerType === 'touch') {
            return;
        }
        if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || event.detail > 1) {
            return;
        }
        if (event.target && event.target.closest && event.target.closest('.html-file a, .document-download-link')) {
            return;
        }
        const fileList = getDataRoomDragScrollContainer();
        if (!fileList) {
        ...
        dataRoomDragStartedKey = getDataRoomItemKey(item);
        ...
        setDataRoomSelection([item], { previewItem: item, focus: false, skipHistory: true });
