(function (global) {
    ...
    function encodeFileUrl(value) {
        const raw = String(value || '');
        const absoluteMatch = raw.match(/^([a-z][a-z0-9+.-]*:\/\/[^/]*)(\/?)(.*)$/i);
        if (absoluteMatch) {
            const [, origin, slash, path] = absoluteMatch;
            return `${origin}${slash ? '/' : ''}${path.split('/').map(encodePathSegment).join('/')}`;
        }
        return raw.split('/').map(encodePathSegment).join('/');
    }
