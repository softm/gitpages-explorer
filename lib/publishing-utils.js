;(function (window, document) {
    ...
    function createToggleButton(manager, item, options = {}) {
        ...
        const refresh = () => {
            ...
            button.textContent = status.isPrivate ? '미게시' : '게시';
            button.title = status.matchedGrantPath && status.matchedGrantPath !== status.path
                ? `현재 미게시 · ${status.matchedGrantPath} 게시로 변경`
                : (status.isPrivate ? '현재 미게시 · 클릭하면 게시' : '현재 게시 · 클릭하면 미게시'); // SOFTM-publishing-state-label 2026-07-09: 버튼 문구를 현재 게시 상태 기준으로 표시
