;(function (window, document) {
    ...
    function createManager(options = {}) {
        ...
        const replaceGrantPaths = (paths, options = {}) => { // SOFTM-publishing-remote-grant 2026-07-09: 원격/로컬 grant.json 경로를 화면 상태 기준선으로 교체
            grantLoaded = options.loaded !== false;
            privatePaths = new Set(uniqueSorted(paths || []));
        ...
        const isPrivatePath = (path, security) => {
            ...
            if (Array.from(privatePaths).some(privatePath => pathMatchesPrivatePath(normalized, privatePath))) {
                return true;
            }
        ...
        const getStatus = (path, security) => {
            ...
            const privateBySecurity = !grantLoaded && Boolean(security && security.private);
        ...
        async function loadLocalGrant() {
            try {
                ...
                replaceGrantPaths(data.private_paths || []);
                if (getToken()) {
                    return loadRemoteGrant({ silent: true, keepLocalOnError: true });
                }
                setStatus('', '');
        ...
        async function loadRemoteGrant(options = {}) {
            ...
            try {
                ...
                replaceGrantPaths(remote.grant.private_paths || []);
        ...
        async function saveToGitHub() {
            ...
            replaceGrantPaths(grant.private_paths || []);
