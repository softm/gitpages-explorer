;(function (window, document) {
    ...
    function createManager(options = {}) {
        ...
        const setPublished = (path, published) => {
            ...
            if (published && hasPrivateSuffix(normalized)) {
                setStatus('-private 이름 규칙으로 미게시된 항목은 이름 변경 후 공개할 수 있습니다.', 'error');
                return false;
            }
            if (published) {
    ...
    function createToggleButton(manager, item, options = {}) {
        ...
        const refresh = () => {
            ...
            button.textContent = status.isPrivate ? '미게시' : '공개';
            button.title = status.matchedGrantPath && status.matchedGrantPath !== status.path
                ? `${status.matchedGrantPath} 퍼블리싱 ON으로 변경`
                : (status.isPrivate ? '퍼블리싱 ON으로 변경' : '퍼블리싱 OFF로 변경');
