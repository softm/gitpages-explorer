(function (window, document) {
    ...
    function buildOfficeViewerUrl(fileUrl) {
        const absoluteUrl = normalizeOfficeSourceUrl(buildAbsoluteUrl(fileUrl));
    ...
    function normalizeOfficeSourceUrl(url) {
        try {
            const parsed = new URL(url, window.location.href);
            if (parsed.hostname.endsWith('.github.io')) {
                const owner = parsed.hostname.replace(/\.github\.io$/i, '');
                const segments = parsed.pathname.replace(/^\/+/, '').split('/');
                const repo = segments.shift();
                if (owner && repo && segments.length > 0) {
                    return `https://raw.githubusercontent.com/${owner}/${repo}/main/${segments.join('/')}`;
                }
            }
        } catch (error) {
            return url;
    ...
    function encodeOfficeViewerSource(url) {
        ...
        params.set('src', decodeUrlForOfficeViewer(url));
        return params.toString().replace(/^src=/, '').replace(/[!'()*]/g, (char) => (
            `%${char.charCodeAt(0).toString(16).toUpperCase()}`
        ));
    }

    function decodeUrlForOfficeViewer(url) {
        try {
    ...
    function render(pane, fileUrl, fileExt, fileName, meta) {
        ...
        if (officeDocumentExtensions.has(normalizedExt) && !isLocalPage()) {
            return renderOfficeDocument(pane, fileUrl, normalizedExt);
        }
