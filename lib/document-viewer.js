(function (window, document) {
    ...
    function officeViewerPathFromMeta(meta) {
        const officeViewer = meta && (meta.officeViewer || meta.office_viewer);
        if (officeViewer && officeViewer.available && officeViewer.path) {
            return officeViewer.path;
        }
        return '';
    }
    ...
    function renderOfficeDocument(pane, fileUrl, fileExt, meta) {
        const officeViewerPath = officeViewerPathFromMeta(meta);
        const sourceUrl = officeViewerPath || fileUrl;
        const viewerUrl = buildOfficeViewerUrl(sourceUrl);
        if (!viewerUrl) {
            createFallback(
                pane,
                fileUrl,
    ...
    function render(pane, fileUrl, fileExt, fileName, meta) {
        ...
        if (normalizedExt === 'pptx' && !isLocalPage() && officeViewerPathFromMeta(meta)) {
            return renderOfficeDocument(pane, fileUrl, normalizedExt, meta);
        }
        if (normalizedExt === 'pptx') {
            renderPptx(pane, fileUrl);
            return true;
        }
        if (officeDocumentExtensions.has(normalizedExt) && !isLocalPage()) {
            return renderOfficeDocument(pane, fileUrl, normalizedExt, meta);
        }
        if (previewPath && (normalizedExt === 'hwp' || normalizedExt === 'hwpx')) {
            renderTextPreview(pane, previewPath);
            return true;
        }
        if (officeDocumentExtensions.has(normalizedExt)) {
            return renderOfficeDocument(pane, fileUrl, normalizedExt, meta);
        }
        if (zipTextExtensions.has(normalizedExt)) {
            renderZipTextDocument(pane, fileUrl, normalizedExt);
            return true;
        }
        if (hwpHtmlExtensions.has(normalizedExt)) {
            renderHwp(pane, fileUrl);
            return true;
        }
        return false;
    }
